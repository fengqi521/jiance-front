<template>
  <div id="electric_profit_charts" class="section-charts"></div>
</template>
...
export default {
  ...
  data () {
    ...
    return {
      electric_profit_charts: ""
    };
  },
  beforeDestroy () {
    window.removeEventListener("resize", () => {
      this.electric_profit_charts.resize();
    });
  },
  methods: {
    //处理数据
    ...
    showEcharts (data) {
      var that = this;
      this.electric_profit_charts = echarts.init(
        document.getElementById("electric_profit_charts")
      );
      var option = that.getCommonConf(data);
      that.electric_profit_charts.clear();
      that.electric_profit_charts.setOption(option);
      // 自定义图标置灰
      that.electric_profit_charts.on("legendselectchanged", (data) => {
        ...
        this.electric_profit_charts.setOption(option);
      });
      window.addEventListener("resize", () => {
        this.electric_profit_charts.resize();
      });
    },
    showElectricProfitChart (data) {
