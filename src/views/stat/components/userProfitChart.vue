<template>
  <div id="user_profit_charts" class="section-charts"></div>
</template>
...
export default {
  ...
  beforeDestroy () {
    window.removeEventListener("resize", () => {
      this.user_profit_charts.resize();
    });
  },
  methods: {
    //处理数据
    ...
    showEcharts (data) {
      var that = this;
      this.user_profit_charts = echarts.init(
        document.getElementById("user_profit_charts")
      );
      var option = that.getCommonConf(data);
      that.user_profit_charts.clear();
      that.user_profit_charts.setOption(option);
      // 自定义图标置灰
      that.user_profit_charts.on("legendselectchanged", (data) => {
        ...
        this.user_profit_charts.setOption(option);
      });
