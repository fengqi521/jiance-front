<template>
    <section>
        <div class="section-title">
            <router-link :to="`/stat/trad/bill?title=${title}&operator_id=${operator_id}`">账单管理</router-link>
            > 账单详情
        </div>
                ...
                <div class="header-main">
                    <div class="header-amount">
                        <div class="top-item">
                            ...
                            <div class="top-item-text ellipsis" style="color: #666">{{ detail.out_trade_id || '--'}}</div>
                        </div>
                        <div class="top-item">
                            <div class="top-item-tr">
                                <div class="top-item-title">账单状态</div>
                                <div :class="['top-item-text ellipsis', detail.payment_status == 1 ? 'section-running' : 'section-no-begin']" style="font-size: 16px">{{ (detail.payment_status) ? (detail.payment_status == 1 ? '已支付' : '未支付') : '--' }}</div>
                            </div>
                            <div class="top-item-tr">
                                <div class="top-item-title">支付时间</div>
                                <div class="top-item-text ellipsis" :style="{ color: detail.payment_status == 1 && detail.payment_time ? '#666' : '' }">{{ detail.payment_status == 1 && detail.payment_time ? detail.payment_time : '--' }}</div>
                            </div>
                        </div>
                        <div class="top-item">
                            <div class="top-item-tr">
                                <div class="top-item-title">实付（元）</div>
                                <div class="top-item-text ellipsis" :style="{ color: parseFloat(detail.real_amount) ? '#666' : '' }">{{ detail.real_amount }} 元</div>
                            </div>
                            <div class="top-item-tr">
                ...
                <div class="section-list">
                    ...
                    <div class="header-main">
                        <div class="header-content clearfix">
                            ...
                            <div class="content-item">
                                <span>所属区域：</span>
                                {{ station.p_name ? station.p_name : '--' }}{{ station.c_name ? station.c_name : '' }}
                            </div>
                            <div class="content-item">
                                <span>详细地址：</span>
                ...
                <div class="section-list">
                    ...
                    <div class="header-main">
                        <div class="header-content clearfix">
                            ...
                            <div class="content-item" v-if="(detail.order_type == 1 || detail.order_type == 4 || detail.order_type == 5 || detail.order_type == 7) && user.user_type == 1">
                                <span>用户类型：</span>
                                个人
                            </div>
                            <div class="content-item" v-else-if="(detail.order_type == 1 || detail.order_type == 4 || detail.order_type == 5 || detail.order_type == 7) && user.user_type == 2">
                                <span>用户类型：</span>
                                企业主用户
                            </div>
                            <div class="content-item" v-else-if="(detail.order_type == 1 || detail.order_type == 4 || detail.order_type == 5 || detail.order_type == 7) && user.user_type == 3">
                                <span>用户类型：</span>
                                特权用户
                            </div>
                            <div class="content-item" v-else-if="(detail.order_type == 1 || detail.order_type == 4 || detail.order_type == 5 || detail.order_type == 7) && user.user_type == 4">
                                <span>用户类型：</span>
                                企业子用户
                            </div>
...
export default {
    ...
    methods: {
        getDetail() {
            ...
            var that = this
            this.$api.billDetail({ out_trade_id: this.id, month: this.month, operator_id: this.operator_id }).then(res => {
