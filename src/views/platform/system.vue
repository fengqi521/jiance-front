<template>
  <section>
    <div class="section-title">运营成员</div>
    <div class="section-tips">
        <i class="warning-icon"></i>
        <span class="tips-text">注意：协助超级管理员管理平台的，由超级管理员创建并分配角色权限。</span>
    </div>
            ...
            >
              ...
              <div class="table-td table-operate">
                ...
                >
                  ...
                  :to="`/platform/system/assign/${item.id}/${item.account}`"
                  ></router-link>
                </el-tooltip>
              </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    // 确认删除
    saveDeleteSure() {
      this.$api.roleDelUser({ id: this.delete_id }).then(data => {
        this.delete_id = "";
    ...
    getAllRoleName() {
      this.$api.roleRoleName({ type: 1 }).then(data => {
        if (data.code == 1) {
          let _data = data.data;
    ...
    getTableDataList() {
      ...
      this.$api.roleSystems(this.modelObj).then(data => {
      ...
      if (id == 0) {
        ...
        this.$refs["modelInfo"].validate(valid => {
          if (valid) {
            ...
            this.$api.roleCreateUser(this.modelInfo).then(data => {
        ...
        this.$refs["modelInfo"].validate(valid => {
          if (valid) {
            ...
            this.$api.roleEditUser(this.modelInfo).then(data => {
              this.showAddUser = false;
              if (data.code == 1) {
                this.$sfNotify({
                  message: "成功"
