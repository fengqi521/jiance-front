<template>
  <section>
    <div class="section-title">
      <router-link to="/platform/system">运营成员</router-link> > 关联充电站
    </div>
    <el-form
      :model="modelInfo"
      ref="modelInfo"
  ...
  methods: {
    saveCreateRole() {
      this.$refs["modelInfo"].validate(valid => {
        if (valid) {
          this.$api.roleAddSystemAuth(this.modelInfo).then(data => {
            if (data.code == 1) {
              this.$sfNotify({
                message: "成功"
              });
              this.$router.push(`/platform/system`);
            } else {
              this.$sfNotify({
                duration: 0,
                type: "error",
    ...
    getRoleTypeRules(id) {
      let that = this;
      this.$api.roleSystemAuth({ id: id }).then(res => {
        if (res.code == 1) {
          let that = this;
          let _data = res.data;
