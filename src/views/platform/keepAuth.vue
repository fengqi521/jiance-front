<template>
  <section>
    <div class="section-title">
      <router-link to="/platform/keep">运维成员</router-link> > 关联充电站
    </div>
    <el-form
      :model="modelInfo"
      ref="modelInfo"
    >
      ...
      >
        ...
        <div class="section-main" v-loading="listLoading">
          <div
  ...
  methods: {
    saveCreateRole() {
      this.$refs["modelInfo"].validate(valid => {
        if (valid) {
          this.$api.roleAddRunningAuth(this.modelInfo).then(data => {
            if (data.code == 1) {
              this.$sfNotify({
                message: "成功"
              });
              this.$router.push(`/platform/keep`);
            } else {
              this.$sfNotify({
                duration: 0,
                type: "error",
    ...
    getRoleTypeRules(id) {
      ...
      this.$api.roleRunningAuth({ id: id }).then(res => {
