...
export default {
  ...
  //        this.user_online_charts.resize();
  //      });
  // },
  beforeDestroy() {
  ...
  methods: {
    ...
    showEcharts(data) {
      var that = this;
      this.user_online_charts = echarts.init(
        document.getElementById("user_online_charts")
      );
      var option = that.getCommonConf(data);
      that.user_online_charts.clear();
      that.user_online_charts.setOption(option);
      window.addEventListener("resize", () => {
        this.user_online_charts.resize();
      });
    },
    showChargeUserOnline(data) {
