<template>
  <div id="fault_pie" class="section-charts"></div>
</template>
...
export default {
  ...
  methods: {
    ...
    showEcharts(data) {
      var that = this;
      this.fault_pie = echarts.init(document.getElementById("fault_pie"));
      var option = that.getCommonConf(data);
      that.fault_pie.clear();
      that.fault_pie.setOption(option);
      that.fault_pie.on("legendselectchanged", (data) => {
        ...
        this.fault_pie.setOption(option);
      });
