<template>
  <div>
    <div
      id="station_online_charts"
      class="section-online-charts"
    ></div>
...
export default {
  ...
  //     this.station_online_charts.resize();
  //   });
  // },
  beforeDestroy() {
  ...
  methods: {
    ...
    showEcharts(data) {
      var that = this;
      this.station_online_charts = echarts.init(
        document.getElementById("station_online_charts")
      );
      var option = that.getCommonConf(data);
      that.station_online_charts.clear();
      that.station_online_charts.setOption(option);
      window.addEventListener("resize", () => {
        this.station_online_charts.resize();
      });
    },
    showChargeStationOnline(data) {
