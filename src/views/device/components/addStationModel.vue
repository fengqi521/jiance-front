    ...
    >
      <div class="box-outer">
        <img
          ...
          @click="handleCancel"
        />
        <div class="box-title">选择充电站</div>
        <div class="box-search">
          <el-input
            v-model="words"
            placeholder="请输入站ID/站名称"
          />
          <span
            class="btn_succ"
            @click="searchOwners"
        ...
        <div class="box-main">
          <div class="table">
            <div class="table-th clearfix">
              ...
              <div class="table-td table-column__1">站ID-站名称</div>
            ...
            <div class="table-body scrollBar">
              ...
              >
                ...
                <div class="table-td table-column__1">{{item.station_id}}-{{item.station_name}}</div>
                <div class="table-td">
                  <el-radio
                    v-model="radio_id"
                    :value="item.station_id"
                    :label="index"
                    @change="stationChangeSelect(index)"
                  ></el-radio>
                </div>
              </div>
            </div>
...
export default {
  ...
  data() {
    return {
      ...
      radio_id: "-1",
      tableData: [],
      saveInfoIndex: [] //点中的是哪个index，为提交做准备
    };
  },
  // watch: {
  //   'source': function (value) {
  //     if (value) this.getStationList()
  //   }
  // },
  created() {
    this.getStationList();
  ...
  methods: {
    ...
    getStationList() {
      ...
      this.$api.pileStations(params).then(res => {
        if (res.code == 1) {
    ...
    stationChangeSelect(index) {
      console.log('00',this.tableData[index]);
      this.saveInfoIndex = index;
    },
    handleCancel() {
      let index = this.saveInfoIndex;
