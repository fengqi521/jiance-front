    ...
    <section>
        ...
        <div class="section-header">
            ...
            <div class="header-content clearfix">
                ...
                <div class="content-item">
                    <div class="header-left">接入运营商</div>
                    <div class="header-right">
                        <el-select v-model="modelObj.operator_id" filterable placeholder="请选择">
                            <el-option v-for="(it, index_o) in operationList" :key="index_o" style="width: 148px" :label="it.title" :value="it.operator_id"></el-option>
                        </el-select>
                    </div>
                </div>
                <div class="content-item">
                ...
                <div class="content-item">
                    ...
                    <div class="header-right">
                        <el-select v-model="modelObj.status">
                            ...
                            <el-option v-for="({ label, id }, index) in status" :key="index" :label="label" :value="id" />
                        </el-select>
                    </div>
                </div>
                <div class="content-item">
                ...
                <div class="table" v-loading="listLoading">
                    ...
                    <div class="table-body">
                        <div class="table-tr" v-show="tableData && tableData.length > 0" v-for="(item, index) in tableData" :key="index">
                            <div class="table-td">
                                <div class="td-item">{{ item.out_trade_id ? item.out_trade_id : '--' }}</div>
                            </div>
                            <div class="table-td">
                            ...
                            <div class="table-td table-column__1">
                                <div class="td-item">{{ item.from_operator ? item.from_operator : '--' }}</div>
                            </div>
                            <div class="table-td table-column__1">
                            ...
                            <div class="table-td">
                                <div :class="['td-item', item.refund_status === '1' ? 'section-no-begin' : '']">
                                    {{ item.refund_status_text }}
                                </div>
                            </div>
                            <div class="table-td">
                                <div class="td-item">{{ item.operator ? item.operator : '--' }}</div>
                            </div>
                            <div class="table-td">
                                <div class="td-item td-time">{{ item.handle_time ? item.handle_time : '--' }}</div>
                            </div>
                            <div class="table-td table-operate">
                                <el-tooltip content="查看" placement="top">
                                    <router-link target="_blank" :to="`/trad/service/detail?out_trade_id=${item.out_trade_id}&month=${modelObj.start_time}`" class="td-item see-icon"></router-link>
                                </el-tooltip>
...
export default {
    ...
    methods: {
        ...
        getOperation() {
            this.$api.pileOperations({ type: 2 }).then(res => {
                if (res.code == 1) {
        ...
        downloadDetail(serial) {
            if (this.is_export) {
                ...
                window.location.href = `${this.ENV}/service/import-list?${params.substr(1)}`
            } else {
        ...
        handleRefund(item) {
            ...
            this.$api.serviceRefundDetail({ out_trade_id: item.out_trade_id }).then(res => {
                if (res.code == 1) {
