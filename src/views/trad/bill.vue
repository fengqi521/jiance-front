  ...
  <section>
    ...
    <div class="section-header">
      ...
      <div class="header-content clearfix">
        ...
        <div class="content-item">
          <div class="header-left">第三方运营商</div>
          <div class="header-right">
            <el-select
                    v-model="modelObj.operator_type"
                    @change="changeOperationSelect"
            >
              <el-option label="全部" value="all" />
              <el-option label="运营商授权" value="3" />
              <el-option label="运营商接入" value="4" />
            </el-select>
          </div>
        </div>
        <div class="content-item">
          <div class="header-left">运营商名称</div>
          <div class="header-right">
            <el-select v-model="modelObj.operator_id">
              <el-option
                      v-for="(it, index_o) in operationList"
                      :key="index_o"
                      style="width: 148px"
                      :label="it.title"
                      :value="it.operator_id"
              >
              </el-option>
            </el-select>
          </div>
          ...
          <div class="table-body">
            ...
            >
              ...
              <div class="table-td">
                ...
                <div
                        class="td-item"
                        v-else-if="item.order_type == 2 || item.order_type == 8"
                >
                  {{ item.user_id }}
                </div>
              ...
              <div class="table-td">
                <div class="td-item" v-if="item.order_type == 6">曹操出行</div>
                <div class="td-item" v-else>
                  {{ item.operator_type_c ? item.operator_type_c : "--" }}
                </div>
              </div>
              <div class="table-td">
              ...
              <div class="table-td">
                <div class="td-item" v-if="item.payment_status == 1">
                  已支付
                </div>
              ...
              <div class="table-td">
                <el-tooltip content="查看" placement="top">
                  <router-link
                          target="_blank"
                          :to="`/trad/bill/detail?id=${item.out_trade_id}&month=${modelObj.start_time}`"
...
<script>
  export default {
    ...
    methods: {
      ...
      getOperation(type) {
        this.$api.pileOperations({ type: type }).then((res) => {
          if (res.code == 1) {
