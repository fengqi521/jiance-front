<template>
    <section>
        <div class="section-title">
            <router-link to="/trad/service">售后管理</router-link>
            > 退款详情
        </div>
        <div class="section-list">
            ...
            <div class="header-main" v-loading="listLoading">
                <div class="header-amount">
                    ...
                    <div class="top-item">
                        <div class="top-item-tr">
                            ...
                            <div
                                :class="[
                                    'top-item-text ellipsis',
                                    {
                                        'section-running': refund_status === '4',
                                    },
                                    { 'section-no-begin': refund_status === '2' || refund_status === '5' },
                                ]"
                                style="fontsize: 16px"
                            >
                                <!-- {{service_status == 1?'待处理':(handle_status == 0?'退款失败':'退款成功')}} -->
                                {{ refund_status_text }}
                            </div>
                        </div>
                        <div class="top-item-tr">
                ...
                <div class="header-content header-content__1">
                    ...
                    <div class="content-item">
                        <span>退款失败原因：</span>
                        <span>{{ refund_fail_reason ? refund_fail_reason : '--' }}</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        getDetail() {
            ...
            this.$api.serviceDetail(params).then(res => {
                ...
                if (res.code == 1) {
                    ...
                    this.refund_fail_reason = _service.refund_fail_reason
                    this.refund_status_text = _service.refund_status_text
                    this.refund_status = _service.refund_status
                }
            })
        },
    },
