<template>
  <section>
    <div class="section-title">
      <router-link to="/trad/profit">业主分润管理</router-link> > 分润账单详情
    </div>
    <div class="section-list">
      ...
      <div class="header-main">
        ...
        <div class="header-content clearfix">
          ...
          <div class="content-item"><span>总费用（元）：</span>{{total_amount}}</div>
          <div class="content-item"><span>优惠折扣（元）：</span>{{bonus_money}}</div>
          <div class="content-item"><span>特权折扣抵扣（元）：</span>{{discount_money}}</div>
          <div class="content-item"><span>总服务费（元）：</span>{{service_amount}}</div>
          <div class="content-item"><span>电费（元）：</span>{{charge_amount}}</div>
        </div>
        <div
...
export default {
  ...
  methods: {
    getProfitDetailInfo() {
      this.$api.profitDetail(this.modelObj).then(data => {
        if (data.code == 1) {
...
@import "../../assets/css/trad/profitDetail.scss";
</style>
